...
import { Plus, Search, Edit, Trash2, GraduationCap, Users, X, BookOpen, ChevronRight } from "lucide-react";
...
const EMPTY_FORM: ClassFormData = {
  name: "", grade: "", section: "", totalStudents: 25, subjects: [],
};
...
export default function Classes() {
  ...
  const upsertMutation = useMutation({
    mutationFn: async (data: ClassFormData) => {
      ...
      if (editing) {
        await apiRequest("PATCH", `/api/classes/${editing.id}`, payload);
      } else {
  ...
  const openEdit = async (cls: Class) => {
    ...
    try {
      const r = await fetch(`/api/classes/${cls.id}/subjects`);
      if (r.ok) {
    ...
    setForm({
      ...
      grade: cls.grade || "",
      section: cls.section || "",
      totalStudents: cls.totalStudents || 25,
      subjects: subs,
    });
    setActiveTab("info");
    setOpen(true);
  ...
  const addSubject = (subjectId: number) => {
    ...
    setForm(p => ({
      ...p,
      subjects: [...p.subjects, { subjectId, teacherId: null, weeklyHours: sub?.weeklyHours || 2 }],
    }));
  };

  const removeSubject = (subjectId: number) => {
    setForm(p => ({ ...p, subjects: p.subjects.filter(s => s.subjectId !== subjectId) }));
  };

  const updateSubjectField = (subjectId: number, field: "teacherId" | "weeklyHours", value: any) => {
    setForm(p => ({
      ...p,
      subjects: p.subjects.map(s => s.subjectId === subjectId ? { ...s, [field]: value } : s),
    }));
  };
        ...
        <CardContent>
          {isLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {Array(10).fill(0).map((_, i) => <div key={i} className="h-32 bg-gray-100 animate-pulse rounded-xl" />)}
            </div>
          ) : filtered.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  ...
                  <div key={cls.id} className="group border border-gray-100 rounded-xl p-4 hover:border-blue-200 hover:shadow-sm transition-all bg-white">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => openEdit(cls)}>
                    ...
                    <div className="flex items-center space-x-1 mt-1 text-gray-400">
                      ...
                      <span className="text-xs">{cls.totalStudents} o'quvchi</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 font-medium">
                {search ? "Qidiruv bo'yicha natija topilmadi" : "Sinflar ro'yxati bo'sh"}
              </p>
              {!search && <p className="text-sm text-gray-400 mt-1">Yangi sinf qo'shish uchun yuqoridagi tugmani bosing</p>}
            </div>
          )}
        </CardContent>
      </Card>
                    ...
                    <div key={sa.subjectId} className="border border-gray-100 rounded-lg p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <div className="w-2 h-2 rounded-full bg-blue-500" style={{ backgroundColor: subjects.find(s => s.id === sa.subjectId)?.color || "#3B82F6" }} />
                          <span className="text-sm font-medium text-gray-900">{subjectName(sa.subjectId)}</span>
                        </div>
                        <button onClick={() => removeSubject(sa.subjectId)} className="text-gray-400 hover:text-red-500 transition-colors">
                          <X className="h-3.5 w-3.5" />
                        </button>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <Select
                            value={sa.teacherId ? String(sa.teacherId) : "none"}
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            value={sa.weeklyHours}
                            onChange={e => updateSubjectField(sa.subjectId, "weeklyHours", parseInt(e.target.value) || 2)}
            ...
            <Button
              onClick={() => {
                if (!form.name && !form.grade) {
                  toast({ title: "Xatolik", description: "Sinf nomi kiritilishi shart", variant: "destructive" });
                  return;
                }
